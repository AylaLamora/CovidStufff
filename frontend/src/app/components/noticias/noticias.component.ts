...
export class NoticiasComponent implements OnInit {
  ...
  clientSearch(name: string, form) {
    ...
    for(let drink of this.datosInventarioService.info){
      if(drink.nombreCli==this.selectedCliente){
      if(this.ab){
        this.arreCate.push([drink.categoria.toString()]);
        this.ab=false;
      }
      this.abc=true;
        for(let a of this.arreCate){
         
          if(drink.categoria.toString()==a.toString()){
            this.abc=false;
          }        
        }
        if(this.abc){
  ...
  agregarArticulo(form?: NgForm) {
    this.cod2 = form.value.codReceta;
  ...
  setCodigoReceta(name: string) {
    // Modifica el nombre de la variable  del servicio
    this.elcodReceta = name; //Captura el codigo
  }
} //AQUI CIERRA TODO
